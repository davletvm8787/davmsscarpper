"""
jobspy.scrapers.kimeta
~~~~~~~~~~~~~~~~~~~

This module contains routines to scrape Kimeta.
"""
...
from ..exceptions import KimetaException
from ..utils import (
    create_session,
    markdown_converter,
)
...
logger = create_logger("Kimeta")


class KimetaScraper(Scraper):
    def __init__(
        self, proxies: list[str] | str | None = None, ca_cert: str | None = None
    ):
        """
        Initializes KimetaScraper with the Kimeta job search url
        """
        site = Site(Site.KIMETA)
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        """
        Scrapes Kimeta for jobs with scraper_input criteria.
        :param scraper_input: Information about job search criteria.
        :return: JobResponse containing a list of jobs.
        """
        self.scraper_input = scraper_input
        self.scraper_input.results_wanted = min(900, scraper_input.results_wanted)
        self.base_url = self.scraper_input.country.get_kimeta_url()
        ...
        if location_type is None:
            logger.error("Kimeta: location not parsed")
    ...
    def _fetch_jobs_page(
        ...
        """
        Scrapes a page of Kimeta for jobs with scraper_input criteria
        """
        jobs = []
        self.scraper_input = scraper_input
        try:
        ...
        with ThreadPoolExecutor(max_workers=self.jobs_per_page) as executor:
            ...
            for future in as_completed(future_to_job_data):
                try:
                    ...
                    raise KimetaException(f"Kimeta generated an exception: {exc}")
    ...
    def _get_location(self, location: str, is_remote: bool) -> (int, str):
        ...
        if res.status_code != 200:
            if res.status_code == 429:
                err = f"429 Response - Blocked by Kimeta for too many requests"
                logger.error(err)
                return None, None
            else:
                err = f"Kimeta response status code {res.status_code}"
                err += f" - {res.text}"
                logger.error(f"Kimeta response status code {res.status_code}")
                return None, None
        items = res.json()

        if not items:
            raise ValueError(f"Location '{location}' not found on Kimeta")
