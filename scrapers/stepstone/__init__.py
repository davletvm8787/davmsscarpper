...
This module contains routines to scrape Stepstone.
"""
...
logger = create_logger("Stepstone")


class StepstoneScraper(Scraper):
    base_url = "https://www.stepstone.de"
    api_url = "https://api.stepstone.de"

    def __init__(
        self, proxies: list[str] | str | None = None, ca_cert: str | None = None
    ):
        """
        Initializes StepstoneScraper with the Stepstone job search url
        """
        super().__init__(Site.STEPSTONE, proxies=proxies)
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        """
        Scrapes Stepstone for jobs with scraper_input criteria.
    ...
    def _find_jobs_in_page(
        ...
        """
        Scrapes a page of Stepstone for jobs with scraper_input criteria
        :param scraper_input:
        ...
        try:
            ...
            if res.status_code not in range(200, 400):
                if res.status_code == 429:
                    err = "429 Response - Blocked by Stepstone for too many requests"
                else:
                    err = f"Stepstone response status code {res.status_code}"
