...
def create_logger(name: str):
    logger = logging.getLogger(f"DAVMSCRAPPER:{name}")
...
def set_logger_level(verbose: int = 2):
    ...
    if level is not None:
        for logger_name in logging.root.manager.loggerDict:
            if logger_name.startswith("DAVMSCRAPPER:"):
    ...
    if match:
        ...
        if (
            ...
            if enforce_annual_salary:
                return interval, annual_min_salary, annual_max_salary, "EURO"
            else:
                return interval, min_salary, max_salary, "EURO"
    return None, None, None, None
